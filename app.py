...
from src.simple_dwg_parser import SimpleDWGParser as DWGParser
...
def run_ai_analysis(box_length, box_width, margin, confidence_threshold,
    ...
    """Run advanced AI analysis with timeout protection"""
    try:
        # Progress tracking
        progress_bar = st.progress(0)
        status_text = st.empty()
        ...
        status_text.text("Analyzing room types with AI...")
        progress_bar.progress(30)
        room_analysis = analyzer.analyze_room_types(st.session_state.zones)

        # Step 3: Placement analysis
        status_text.text("Calculating optimal furniture placements...")
        progress_bar.progress(60)
        params = {
        ...
        # Step 4: Optimization
        status_text.text("Running spatial optimization...")
        progress_bar.progress(80)
        ...
        st.session_state.analysis_results = {
            ...
            'zones_analyzed': len(room_analysis),
            'efficiency_score': optimization_results.get('total_efficiency', 0.85)
        }
        ...
        st.success(
            f"🎉 Advanced AI Analysis Complete!\n"
            f"📊 Analyzed {len(room_analysis)} zones\n"
            f"📦 Found {total_boxes} optimal placements\n"
            f"⚡ Efficiency: {optimization_results.get('total_efficiency', 0.85)*100:.1f}%"
        )

    except Exception as e:
        st.error(f"❌ Analysis error: {str(e)}")
        # Create minimal results to prevent app crash
        st.session_state.analysis_results = {
            'rooms': {'Zone_0': {'type': 'Office', 'confidence': 0.5, 'area': 100}},
            'placements': {'Zone_0': [{'position': (0, 0), 'size': (box_length, box_width)}]},
            'optimization': {'total_efficiency': 0.5},
            'parameters': {'box_size': (box_length, box_width)},
            'total_boxes': 1,
